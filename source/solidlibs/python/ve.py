'''
    ...
    No more shell script wrappers with "cd VIRTUALENV_DIR ; source bin/activate".
    Just pure python scripts using virtualenv and this module.
...
def venv(dirname=None, django_app=None, restore=True):
    ...
    debug(f've.activate(dirname={dirname or None}, django_app={django_app or None}, restore={restore or False})')
...
def virtualenv_dir(dirname=None):
    ...
    debug(f'virtualenv_dir(dirname={dirname or None})')
    vdir = None
    ...
    if not vdir:
        if 'VIRTUAL_ENV' in os.environ:
            ...
            debug(f'VIRTUAL_ENV={vdir}')

    if not vdir:
        debug('try dirs of calling modules')

        # we want the last caller which is not this module
        stack = traceback.extract_stack()
        stack_str = ''
        for element in stack:
            elem_str = repr(element)[1:-1]
            stack_str += '\n\t' + repr(element)[1:-1]
        #debug(f"stack: {stack_str}")
