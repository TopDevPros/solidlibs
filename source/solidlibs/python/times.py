...
def parse_timestamp(timestr, startswith=False, default_year=None, timezone=None):
    ''' Parse a string timestamp. Return datetime.
        ...
        'datetime.datetime(2023, 10, 28, 9, 54, 48)'
    ...
    if not timestamp:
        raise ValueError(f"no timestamp found: '{timestamp}'")
...
def date_range(start, end, lei_convention=False):
    ...
    log(f'start: {start}') #DEBUG
    log(f'end: {end}') #DEBUG
    log(f'lei_convention: {lei_convention}') #DEBUG
    increasing = start <= end
    log(f'increasing: {increasing}') #DEBUG
