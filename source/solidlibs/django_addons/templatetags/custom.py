...
def title(*args, size='h1'):
    ...
    if args:
        text = args[0]
        html = django.utils.html.format_html('<{}> <strong> {} </strong> </{}>', size, text, size)
    else:
        html = django.utils.html.format_html('<{}> <strong> ', size)
    return html

@register.simple_tag
def endtitle(size='h1'):
    ...
    return django.utils.html.format_html('</strong> </{}>', size)
...
def subtitle(*args):
    ...
    return title(*args, size='h4')

@register.simple_tag
def endsubtitle(size='h4'):
...
def start_link_button(href,
    ...
    BaseFormat = '<a href="{}" name="{}" id="{}" class="btn {}{}{}" type="button" {}> <strong>'
...
@register.simple_tag
def card_boxed(*args):
    ''' Our standard version of the bootstrap card with a box around it instead a different background.

        Usage::

            {% card-boxed "My card" %}

        Use 'card-boxed'/'endcard' if there are django template variables in the content
        or the content is more than one line.
    '''

    START_CARD_HTML = '<div class="card"><div class="card-body border rounded">'
