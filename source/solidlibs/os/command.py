...
def run(*command_args, **kwargs):
    ''' Run a command line.
        ...
        If run is called with verbose=True, run() logs extra information.
...
def handle_run_error(command_args, cpe):
    ...
    command_dir, __, program = command_str.rpartition('/')
    if command_dir:
        lines.append(f'command dir: {command_dir}')
    lines.append(f'return code: {cpe.returncode}')
    failed_msg = '\n\t'.join(lines)
    log(failed_msg)

    log(f'CalledProcessError: {format_output(cpe)}')
    try:
...
def wait_child(process):
    ''' Wait for a background process to finish.
        ...
        Simpler approaches didn't work. Neither does this approach.
    ...
    def active(pid):
        return  os.path.isdir(f'/proc/{str(pid)}')

    def get_pids():
        result = run('ps', '-eo', 'pid')
        return result.stdout.split()

    raise RuntimeError('wait_child() not working')
    ...
    while active(process.pid):
        sleep(1)

    """
    # WARNING: Waits for any running program with the same name.
    program_name = process.args[0]
    while is_program_running(program_name):
        sleep(0.1)
    """

    """
    pid = str(process.pid)
    # allow time for pid to register
    while not active(pid):
        log(f'{pid} not active')
        sleep(1)

    while active(pid):
        sleep(1)

    while pid not in get_pids():
        log(f'{pid} not in pids:{get_pids()}')
        sleep(1)
    while pid in get_pids():
        sleep(1)
    """
